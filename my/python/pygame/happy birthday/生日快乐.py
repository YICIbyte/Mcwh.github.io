...
def ClassThread():
    global 音乐区_class, color_music, music_panel, \
        ...
        point_move_Group, eye_1, eye_2, switch, steam
    ...
    for i in range(len(音乐总和_name)):
        ...
        numbers += 1
        音乐按钮总和.append(Music_Pay_Button(音乐总和_name[i], (音乐高度 * i + 45 * bili), color_music, 音乐高度)) # 45是初始第一个音乐按钮的top坐标
...
class Clock:
    def __init__(self, types, center_x):
        ...
        self.rect.center = (self.center_x, height * 0.6)
...
class Password:
    def __init__(self):
        self.mouse_pos = None
        self.image = pygame.Surface((160*bili, 50*bili), pygame.SRCALPHA)
        self.rect = self.image.get_rect(center=(width * 0.5, height * 0.35))
        self.color = BLUE
        self.password = None
        def load_password():
            try:
        ...
        self.font = pygame.font.SysFont("kaiti", round(18*bili))
        self.radius = round(15 * bili)
        self.draw_width = round(5*bili)

        self.image_txt_y = self.font.render("记住密码", True, BLUE)
        self.image_txt_n = self.font.render("忘记密码", True, BLUE)
        self.image_txt = None

        self.rect_txt = self.image_txt_y.get_rect(center=(self.rect.width/2, self.rect.height/2))
    ...
    def is_mouse_over(self):
        ...
        return self.rect.collidepoint(self.mouse_pos)
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and self.is_mouse_over() and not clock1.可滑动 and not clock2.可滑动 and not clock3.可滑动:
            if event.button == 1:
                self.yn = not self.yn
                button_music.play()
    def update(self):

        # 如果选择记住密码，就显示忘记密码，否则显示记住密码
        if self.yn:
            self.image_txt = self.image_txt_n
            # 显示内部按钮
            self.inClorSRCALPHA = 255
        else:
            self.image_txt = self.image_txt_y
            self.inClorSRCALPHA = 0

    def draw(self):
        # 绘制按钮轮廓
        pygame.draw.rect(self.image, self.color, (0, 0, self.rect.width, self.rect.height), border_radius=self.radius, width=self.draw_width)
        # 绘制按钮内部
        pygame.draw.rect(self.image, (255, 128, 200, self.inClorSRCALPHA), ((self.draw_width * 3 / 2, self.draw_width * 3 / 2), (self.rect.width - self.draw_width * 3, self.rect.height - self.draw_width * 3)), border_radius=round(self.radius*0.7))
        # 鼠标在按钮上时，改变轮廓颜色为红色
        if self.is_mouse_over() and not clock1.可滑动 and not clock2.可滑动 and not clock3.可滑动:
            self.color = (255, 0, 0)
            # 绘制文本
            self.image.blit(self.image_txt, self.rect_txt)
        else:
            # 改变轮廓颜色为蓝色
            self.color = BLUE
...
class Unlock:
    def __init__(self):
        ...
        self.image = pygame.Surface(self.size, pygame.SRCALPHA)
        self.rect = self.image.get_rect(center=(width / 2, height * 0.85))
...
class Letter:
    def __init__(self, x, y):
        ...
        self.letters = ("祝福语寿星黄宇涛：咳咳！头一次以这样的方式给你庆生，恭喜啊，20岁生日快乐！祝你长命百岁、百毒不侵、百福不减、家财万贯。"
                        "估摸着认识有7年多了吧，平日里有大大小小的矛盾，打游戏经常把你惹生气，话说还是很感谢你的，虽然平时老损我，"
                        "但在重要时刻还是很力挺我的，学习上对我也有很大的帮助，和你做朋友我很开心。好话坏话今天先说到这儿，剩下的槽点和"
                        "祝福留着明年生日接着唠。你喜欢音乐，也很喜欢周杰伦，所以我就给你做了一个音乐播放器，里面我存了几首他出名的歌曲"
...
class PointMove(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if not ((self.rect.colliderect(button_1.rect) and self.rect.y < button_1.rect.top)
                ...
                or (self.rect.colliderect(switch.rect) and self.rect.y < switch.rect.top)):
            self.y += self.speedy
        # 自杀的4个条件
...
class Music_Panel:
    def __init__(self, 滑条s):
        ...
        self.image_music_container = pygame.Surface(
            (self.width, self.滑条s.音乐加文字高度), pygame.SRCALPHA)
...
class Switch:
    def __init__(self):
        self.size = (70*bili, 30*bili)
        self.image = pygame.Surface(self.size, pygame.SRCALPHA)
        self.rect = self.image.get_rect()
        self.rect.center = (round(width * 0.9), round(height * 0.45))
        self.rect_width = round(5*bili) # 线条宽度
        self.alpha = 100
        self.alpha_speed = 5
        ...
        self.radius = round(15*bili)
        # 圆的x位置范围
        self.circle_x_max = self.rect.width-self.radius-self.rect_width
        self.circle_x_min = self.radius+self.rect_width
        # 圆的中心位置
        self.circle_x = self.circle_x_max
        self.circle_y = round(self.rect.height/2)
    ...
    def update(self):
        ...
        # 如果开关为关
            ...
            self.circle_x -= self.speed
        # 限制透明度范围
        self.alpha = max(100, min(self.alpha, 255))
        # 限制圆的x位置范围
        self.circle_x = max(self.circle_x_min, min(self.circle_x, self.circle_x_max))

    def draw(self):
        self.image.fill((0, 0, 0, 0))
        # 矩形底色
        # pygame.draw.rect(self.image, PURPLE, (0, 0, self.size[0], self.size[1]), border_radius=self.radius, width=self.rect_width)
        # 绘制圆
        pygame.draw.circle(self.image, (255, 180, 100), (self.circle_x, self.circle_y), self.radius)
        # 矩形边框
        pygame.draw.rect(self.image, (102, 187, 255), (0, 0, self.size[0], self.size[1]), border_radius=self.radius, width=self.rect_width)
    ...
    def is_mouse_over(self):
        pos = list(pygame.mouse.get_pos())
        pos[0] -= kuangjia.rect.x
        pos[1] -= kuangjia.rect.y
        return self.rect.collidepoint(pos)
    def handle_event(self, event):
        if self.is_mouse_over() and event.type == pygame.MOUSEBUTTONUP:
            if event.button == 1:
                # 启动开关
                self.yn = not self.yn
                # 按钮音效
...
class GameSpeak:
    def __init__(self):
        ...
        self.rect_text = self.image_text.get_rect(center=(width * 0.8, height/2))
        ...
        self.font_speak = pygame.font.Font(FONT_PATH, round(30*bili))
...
class STEAM:
    def __init__(self):
        self.font_text = pygame.font.Font(FONT_PATH, round(30*bili))

        try:
            with open(folders[16] + "path.dll", "r", encoding="utf-8") as f:
                # 获取链接信息
                self.path_contain = f.read().strip("\n").strip(" ")
                # 获取名字
                self.path_name = self.path_contain.split("\\")[-1]
                # 关闭文件
                f.close()
            # print(111)
        except:
            # 获取桌面路径
            self.path_contain = os.path.join(os.path.expanduser("~"), "Desktop") + "\\Steam"

            self.path_name = self.path_contain.split("\\")[-1]

            with open(folders[16] + "path.dll", "w", encoding="utf-8") as f:
                f.write(self.path_contain)
                f.close()
            # print(222)


        self.image = pygame.Surface((140 * bili, 70 * bili), pygame.SRCALPHA)
        self.rect = self.image.get_rect(center=(width * 0.2, height * 0.4))
        pygame.draw.rect(self.image, BLACK, ((0, 0), self.rect.size), border_radius=round(20*bili))
        self.font_speak = pygame.font.SysFont("kaiti", round(30*bili))
        self.image_speak = self.font_text.render(self.path_name, True, VIOLET)
        self.rect_speak = self.image_speak.get_rect(center=(self.rect.width / 2, self.rect.height / 2))
        self.image.blit(self.image_speak, self.rect_speak)
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONUP:
            if event.button == 1 and self.is_mouse_over():
                ...
                try:
                    # 打开Steam游戏
                    os.startfile(self.path_contain)
                    # 退出程序
                    self.yn = True
                    # print(333)
                except:
...
def 大厅选择screen():
    ...
    if len(point_move_Group) < 400 and switch.return_yn() == True:
        # 判断小球是否超过3秒，超过就从-20开始生成，否则在-height上生成
        if pygame.time.get_ticks() - switch.action_time < 3000:
            point_move_Group.add(PointMove(random.randint(0, round(width)), -height, random.uniform(-2, 2), random.uniform(3, 5.5)))
        else:
            # 之后在-20高度生成
            point_move_Group.add(PointMove(random.randint(0, round(width)), -20, random.uniform(-2, 2), random.uniform(3, 5.5)))
